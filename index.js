...
for (let flower of flowers) {
    ...
    <div><span style = "color: transparent"> <span > Категория: </span>${flower.category}</div>
    </div>
    <br>
...
//меняем цвет кнопки по клику - меняется по клику цвет всех кнопок :)
/*const buttonS = Array.from(document.querySelectorAll('.button_basket'));
console.log(buttonS);
buttonS.forEach(button => addEventListener('click', () => {
    //button.style.backgroundColor = "#0093a2";
    button.style.backgroundColor = "pink";
}))*/


    //меняем цвет кнопки по клику
    document.addEventListener('click', function(event) {
    //проверяем, что клик был совершен по кнопке в корзину
    if(event.target.hasAttribute('data-catalog')) {
    console.log('Hello! я из корзины!');
    //находим карточку с товаром, внутри которой был совершен клик
    const buttonNewColor = event.target.closest('.button_basket');
    buttonNewColor.style.backgroundColor = "#ff3672";}})


     //меняем цвет карточек по клику - карточки меняют цвет на бежевый
     document.addEventListener('click', function(event) {
    //проверяем, что клик был совершен по кнопке в корзину
    if(event.target.hasAttribute('data-catalog')) {
    console.log('я из корзины!');
    //находим карточку с товаром, внутри которой был совершен клик
    const cardNewColor = event.target.closest('.flowers_card');
    //console.log(card);
    cardNewColor.style.backgroundColor = "rgb(243, 220, 206)";}})
